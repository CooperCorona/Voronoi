...
extension ExposedBinarySearchTreeProtocol {
    ...
    */
    public func getNearestLeftChild() -> Self? {
        var child = self.left
        while let node = child where !node.isLeaf {
            child = node.right
        }
        return child
    }
    ...
    public func getLeftmostParent() -> Self? {
        ...
        var cur:Self? = self
        while let left = par?.left where left === cur {
            cur = par
            par = par?.parent
            /*if par == nil {
                return nil
    ...
    public func getRightmostParent() -> Self? {
        ...
        var cur:Self? = self
        while let right = par?.right where right === cur {
            cur = par
            par = par?.parent
            /*if par == nil {
                return nil
    ...
    public func iterateChildren(handler:(Self) -> Void) {
        handler(self)
...
public struct ExposedBinarySearchTree<T: ExposedBinarySearchTreeProtocol>: CustomStringConvertible {
    ...
    public private(set) var root:T? = nil
    ...
    public func find(requireLeaf:Bool, predicate:(T) -> Int) -> T? {
        var current = self.root
    ...
    public func findLeaf(predicate:(T) -> Int) -> T? {
        var current = self.root
        while let cur = current {
            let delta = predicate(cur)
            if delta < 0 {
    ...
    public func iterateLeaves(handler:(T) -> Void) {
        if let root = self.root {
            self.iterateNode(root, requireLeaf: true, handler: handler)
        }
    }
    
    public func iterateNodes(handler:(T) -> Void) {
        if let root = self.root {
            self.iterateNode(root, requireLeaf: false, handler: handler)
        }
    }
    
    private func iterateNode(node:T, requireLeaf:Bool, handler:(T) -> Void) {
    ...
    private static func toString(val:T, depth:Int) -> String {
        var tabs = ""
        for _ in 0..<depth {
            tabs += " "
        }
    ...
    private func calculateCountOf(node:T?, leaves:Bool) -> Int {
        if let node = node {
    ...
    private func calculateLayersOf(node:T?) -> Int {
        if let node = node {
